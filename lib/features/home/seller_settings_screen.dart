...
class _SellerSettingsScreenState extends State<SellerSettingsScreen> {
  ...
  Future<void> _navigateToEditData() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => SellerEditDataScreen(sellerId: widget.sellerId),
      ),
    );

    // Если данные были обновлены, перезагружаем
    if (result == true) {
      _loadSellerData();
    }
  }
  ...
  Future<void> _logout() async {
    try {
      await _apiService.logoutSeller(widget.sellerId);
      if (mounted) {
  ...
  Future<void> _deleteAccount() async {
    try {
      await _apiService.deleteSeller(widget.sellerId);
      if (mounted) {
        Navigator.of(context).pushNamedAndRemoveUntil('/login', (route) => false);
      }
    } catch (e) {
